...
def ql_syscall_close(ql: Qiling, fd: int):
    try:
        ql_fd = ql.os.fd[fd]
        # 如果是 socket 对象，使用 socket.close()
        if hasattr(ql_fd, 'socket') and hasattr(ql_fd.socket, 'close'):
            print(f"[+] Closing socket fd {fd}")
            try:
                ql_fd.socket.close()
                ql.arch.regs.rax = 0
                return 0
...
def ql_syscall_read(ql: Qiling, fd, buf: int, length: int):
    try:
        ql_fd = ql.os.fd[fd]
        # 如果是 socket 对象，使用 socket.recv()
        if hasattr(ql_fd, 'socket') and hasattr(ql_fd.socket, 'recv'):
            print(f"[+] Socket read from fd {fd}, max {length} bytes")
            try:
                data = ql_fd.socket.recv(length)
                if len(data) > 0:
                    ql.mem.write(buf, data)
                    return len(data)
                    print(f"[+] Socket read {len(data)} bytes")
                else:
                    # 连接关闭
                    print(f"[+] Socket connection closed (read returned 0)")
                    ql.arch.regs.rax = 0
                return 0
            except BlockingIOError:
                # 非阻塞 socket，没有数据可读
                print(f"[+] Socket read would block")
                ql.arch.regs.rax = -1
                return -1
            except Exception as e:
...
def ql_syscall_write(ql: Qiling, fd: int, buf: int, count: int):
    try:
        ql_fd = ql.os.fd[fd]
        # 如果是 socket 对象，使用 socket.send()
        if hasattr(ql_fd, 'socket') and ql_fd.socket is not None and hasattr(ql_fd.socket, 'send'):
            data = ql.mem.read(buf, count)
            print(f"[+] Socket write to fd {fd}, {len(data)} bytes")
            print(f"    Data preview: {data[:100]}")
            try:
                # 对于非阻塞 socket，可能需要多次尝试
                total_sent = 0
                while total_sent < len(data):
                    try:
                        sent = ql_fd.socket.send(data[total_sent:])
                        if sent == 0:
                            # 连接关闭
                            print(f"[!] Socket connection closed during send")
                            ql.arch.regs.rax = -1
                            return -1
                        total_sent += sent
                        print(f"    Sent {sent} bytes, total {total_sent}/{len(data)}")
                    except BlockingIOError:
                        # 非阻塞 socket，暂时无法发送
                        import time
                ...
                ql.arch.regs.rax = total_sent
                print(f"[+] Socket write completed: {total_sent} bytes")
                return total_sent
            except Exception as e:
                print(f"[!] Socket send error: {e}")
                import traceback
